    ...
    def __str__(self):
        if len(self.links) >= 1:
            ...
            for i in range(0, len(self.links)):
                string += ' ' + 'INNER JOIN ' + str(self.links[i][1][0]) + ' ' + 'ON ' + str(
                    self.links[i][0][0]) + '.' + str(self.links[i][0][1]) + ' = ' + str(
                    self.links[i][1][0]) + '.' + str(self.links[i][1][1])
            return string
        ...
        if len(self.conditions) >= 1:
            for i in range(0, len(self.conditions)):
                if i == 0:
                    ...
                    string += ' ' + color.BOLD + str(self.conditions[i][0]) + color.END + ' ' + str(
                        self.conditions[i][1])
            return string
        else:
            return string
...
class Query():
    ...
    def __str__(self):
        return str(self.select) + str(self._from) + str(self.join) + str(self.where) + str(self.group_by) + str(
            self.order_by) + ';'
